    ...
    private static void load(Context context, ImageView view, Object objUrl, ImageLoadConfig config, final LoaderListener listener) {
        ...
        try {
            ...
            if (config.isAsGif()) {//gif类型
                ...
                // TODO: 2017/3/12 在这里可以进行图片转换   不过这个需要加入一个转换的第三方库
...
//                }
     ...
     * @param listener  监听器
     */
    public static void loadBitmap(Context context, ImageSize imageSize,Object url, final BitmapLoadingListener listener) {
        if (url == null) {
            if (listener != null) {
                listener.onError();
            }
            ...
            if (imageSize !=null){
                Glide.with(context).
                        load(url).
                        asBitmap().override(imageSize.getWidth(),imageSize.getHeight()).
